...
- `Direct Buy-1z7.6` historical topology decision: Phase 20 smoke stayed PM2/Nginx on app host `91.132.59.194`; central Caddy on `80.74.28.160` terminated HTTPS for `directbuy.aidevteam.ru` and proxied to `91.132.59.194:3001`. Compose remains a future migration, not the current smoke contract.
- 2026-05-09 edge cleanup: by operator request, `directbuy.aidevteam.ru` was removed from `central-caddy` on `80.74.28.160`; Caddy validate/reload passed and live `/etc/caddy/Caddyfile` no longer contains the Direct Buy route. Direct DNS/TLS setup on the app host was completed separately.
- 2026-05-13 current production domain: Direct Buy admin is `https://directbuy.zalogium.ru` on app host `91.132.59.194`; Nginx terminates TLS and proxies to `127.0.0.1:3001`. Production `.env` has `DOMAIN=directbuy.zalogium.ru`, `NEXT_PUBLIC_BOT_USERNAME` points to `mo_lot_bot`, and `@mo_lot_bot` is configured in BotFather for the web login domain.
