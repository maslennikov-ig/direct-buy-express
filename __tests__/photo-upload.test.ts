...
describe('Photo Upload Validation (Phase 16)', () => {
    ...
    it('should allow completion after uploading one photo', async () => {
        const ctx = createMockCtx();
        let callCount = 0;
        const doneCallback = {
            callbackQuery: { data: 'lot_photos_done' },
        ...
        const mockWait = vi.fn().mockImplementation(() => {
            ...
            if (callCount === 9) return doneCallback;

            // Financials (Price)
            if (callCount === 10) return { message: { text: "10000000" } };
            // Urgency
        ...
        expect(createManyCall.data.length).toBe(1);
    });

    it('should automatically proceed after receiving exactly 30 photos', async () => {
        const ctx = createMockCtx();
        let callCount = 0;
        ...
        const mockWait = vi.fn().mockImplementation(() => {
            ...
            // Upload exactly 30 photos
            if (callCount >= 8 && callCount <= 37) return { message: { photo: [{ file_id: `photoX_small` }, { file_id: `photoX_large` }] } };

            // Financials
            if (callCount === 38) return { message: { text: "10000000" } };
            return { message: { text: "Mock urgency" } };
        });
        ...
        expect(ctx.reply).toHaveBeenCalledWith('✅ Загружено 30 фото (максимум). Переходим дальше.');

        const createManyCall = (prisma.media.createMany as any).mock.calls[0][0];
        expect(createManyCall.data.length).toBe(30);
    });
